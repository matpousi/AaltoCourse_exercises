...
def filter_cells(notebook: Dict[str, Any], exclude_tag: str) -> Dict[str, Any]:
  ...
  filtered_cells = [cell for cell in original_cells if not cell_has_tag(cell, exclude_tag)]

  filtered_notebook['cells'] = filtered_cells

  if (len(original_cells) == len(filtered_cells)):
    print(f"No cells with tag {exclude_tag} found, no filtered notebook produced")
    return None
...
def main():
  ...
  parser = argparse.ArgumentParser(description="Remove cells with specified tags from Jupyter notebooks",
                                   epilog="""
Examples:
  ...
  parser.add_argument('path', help='File path, wildcard pattern, or directory containing notebooks')

  parser.add_argument('--tag', '-t', default='remove', help='Tag of cells to exclude (default: "remove")')

  parser.add_argument('--suffix', '-s', default='_filtered', help='Suffix to add to output filenames (default: "_filtered")')
  ...
  for notebook_path in notebooks:
    p = process_notebook(notebook_path, args.tag, args.suffix)
    if p is not None:
      output.append(p)

  print(f"Processing complete! Processed {len(notebooks)} notebook(s) and saved {len(output)} notebook(s).")
  return output
